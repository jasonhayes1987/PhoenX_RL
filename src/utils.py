...
def get_callbacks(callbacks, project):
    ...
    callback_objects = {
        "Weights & Biases": tf_callbacks.WandbCallback(project),
    }
    #DEBUG
    # print(f'callbacks: {callbacks}')

    callbacks_list = [tf_callbacks.DashCallback("http://127.0.0.1:8050")]
    # for id_dict, value in zip(all_ids, all_values):
...
def load(agent_data, env_name):
    ...
    if agent_data['agent_type'] == 'Reinforce' or agent_data['agent_type'] == 'ActorCritic':
        ...
        policy_layers = [(units, activation, tf.keras.initializers.deserialize(initializer)) for (units, activation, initializer) in agent_data['policy_model']['dense_layers']]

        ##DEBUG
        # print("Policy layers:", policy_layers)
        
        value_layers = [(units, activation, tf.keras.initializers.deserialize(initializer)) for (units, activation, initializer) in agent_data['value_model']['dense_layers']]
        ...
        if agent_data['agent_type'] == "Reinforce":
            ...
            agent = rl_agents.Reinforce(
                ...
                callbacks=[tf_callbacks.WandbCallback.load(Path(agent_data['save_dir']).joinpath(Path("wandb_config.json"))) for callback in agent_data['callbacks']],
