...
parser.add_argument('--learner_device', type=str, default='cuda', help='Device for the learner (default: cuda)')
parser.add_argument('--learner_num_cpus', type=int, default=1, help='Number of CPUs for the learner (default: 1)')
parser.add_argument('--learner_num_gpus', type=int, default=1, help='Number of GPUs for the learner (default: 1)')
...
parser.add_argument('--worker_num_gpus', type=int, default=0, help='Number of GPUs for the workers (default: 0)')
parser.add_argument('--learn_freq', type=int, default=1000, help='Learn frequency for the distributed workers (default: 1000)')
parser.add_argument('--sync_interval', type=int, default=100, help='Sync interval for the distributed workers (default: 100)')
            ...
            if agent_type in ['ActorCritic', 'DDPG', 'TD3']:
                if args.distributed_workers > 1:
                    distributed_agents = DistributedAgents(agent_config,
                                                           ...
                                                           args.learn_freq)
                    futures = distributed_agents.train(sync_interval=args.sync_interval, num_episodes=num_episodes, num_envs=num_envs, seed=seed, render_freq=render_freq)
                    if futures:
                        ray.get(futures)
                else:
