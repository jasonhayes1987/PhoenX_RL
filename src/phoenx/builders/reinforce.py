from phoenx.rl_agents import Reinforce
...
def build(config: dict, env: EnvWrapper) -> Reinforce:
    """Build a Reinforce agent from a config.

    Args:
        config (dict): The config to build the Reinforce agent from.
        env (EnvWrapper): The environment to build the Reinforce agent for.

    Returns:
        Reinforce: The built Reinforce agent.
    """
    agent_cfg = config['agent']['config']

    # Canonical 'model:' schema (roots/trunk/branches)
    apply_model_config(agent_cfg, env, "Reinforce")
